...
// budgetController.add(3) = Error budgetController.add is NOT a function!
...
// budgetController and UIController doesn't share data so the appController below make it possible.
...
var budgetController = (function(){ 
	...
	return {
		 ...
		 caluculateBudget: function(){
		 	...
		 	caluculateTotal('exp');
		 	caluculateTotal('inc');
		 	//caluculate the budget incomes - expenses
		 	data.budget = data.totals.inc - data.totals.exp;
		 	//caluculate % of income that we spent
		 ...
		 caluculatePercentages: function(){

		 	// a = 20, b = 10, c = 40;
		 	// income = 100
		 	// a = 20/100 * 100 = 20%, b = 10%, c = 40%
...
var UIController = (function(){
	...
	var formatNumber = function(num, type){
		var numSplit, int, dec, type;
		 	...
		 	num = Math.abs(num);//change number to number
		 	num = num.toFixed(2);//exactly 2 decimal points

		 	numSplit = num.split(".");

		 	int = numSplit[0];//100000
		 	if (int.length > 3){
		...
		clearFields: function(){
		 ...
		 displayPercentages : function(percentages){
		 	...
		 	if (percentages[index]  > 0){
		 		...
		 		document.querySelector(DOMstrings.expencesPercLabel).textContent = "---"
		 	}
		 	});
...
// GROBAL APP CONTROLLER --- CONTROLLER MODULE
var controller = (function(budgetCtrl, UICtrl){
	...
	var setupEventListeners = function(){
		...
		document.addEventListener('keypress', function(event){
			if (event.keyCode === 13 || event.which === 13){ //some browsers use which for keyCode...
				ctrlAddItem();
			}
			//console.log(event);
	...
	var updatePerentages = function(){

		// Caluculate percentages
		budgetCtrl.caluculatePercentages();
